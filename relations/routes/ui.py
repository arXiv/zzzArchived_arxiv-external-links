...
def service_status() -> Response:
    ...
    """
    response_data, status_code, headers = controllers.service_status(MultiDict([]))
...
def create_new(arxiv_id_str: str, arxiv_ver: int) -> Response:
    ...
    response_data, status_code, headers = controllers.create_new(arxiv_id_str,
                                                                 ...
                                                                 request.json)
    response: Response = \
        make_response(render_template("relations/create_new.html", **response_data))
    response.headers.extend(headers)
    response.status_code = status_code
    return response


@blueprint.route('/<string:arxiv_id_str>v<int:arxiv_ver>/relations/<string:relation_id_str>',
                 methods=['POST'])
...
@blueprint.route('/<string:arxiv_id_str>v<int:arxiv_ver>/relations/<string:relation_id_str>/delete',
                 methods=['POST'])
def suppress(arxiv_id_str: str, arxiv_ver: int, relation_id_str: str) -> Response:
    ...
    response_data, status_code, headers = controllers.suppress(arxiv_id_str,
                                                               ...
                                                               request.json)
    response: Response = \
        make_response(render_template("relations/suppress.html",
                                      **response_data))
    response.headers.extend(headers)
    response.status_code = status_code
    return response
...
def get_relations(arxiv_id_str: str, arxiv_ver: int) -> Response:
    ...
    response: Response = \
        make_response(render_template("relations/get_relations.html",
                                      **response_data))
    response.headers.extend(headers)
    response.status_code = status_code
    return response
...
def get_events(arxiv_id_str: str, arxiv_ver: int) -> Response:
    ...
    # may need to include arxiv_id and arxiv_ver here(or have controllers changed)
    response: Response = \
        make_response(render_template("relations/get_events.html", **response_data))
    response.headers.extend(headers)
    response.status_code = status_code
    return response
